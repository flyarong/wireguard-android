...
public class PeerProxy extends BaseObservable {
    ...
    public PeerProxy(final ConfigProxy parent, final Peer peer) {
        ...
        endpoint = peer.getEndpoint().map(PeerProxy::formatEndpoint).orElse("");
    ...
    private static String formatEndpoint(final InetSocketAddress endpoint) {
        final String host = endpoint.getHostString();
        if (host.contains(":") && !host.contains("["))
            return String.format("[%s]:%d", host, endpoint.getPort());
        else
